...
const Mutation = compose(
  ...
  lifecycle({
    componentWillReceiveProps(nextProps: {}): void {
      ...
      console.log(activeTranscript && activeTranscript.transcript_id);
      ...
      if (!this.props.ssms.length || !isEqual(this.props.activeTranscript || canonicalTranscript, activeTranscript)) {
        ...
        nextProps.setState(s => ({ ...s, vizLoading: false }));
      }
    },
  ...
  allCasesAgg,
  gene,
  state: { ProteinLolliplot, vizLoading },
  activeTranscript,
  setActiveTranscript,
}) => {
  ...
  const consquenceDataMapped = Object.values(consequenceData)
    .map(d => ({
      ...d,
      symbol: <GeneLink id={d.gene_id}>{d.symbol}</GeneLink>,
      transcripts: (
        <ul style={{ listStyle: 'none', paddingLeft: 0, marginBottom: 0 }}>
          {d.transcripts.map(t =>
            <li
              key={t}
              style={{ fontWeight: t === d.canonical_transcript_id ? 'bold' : 'normal' }}
            >
              <ExternalLink
                key={t}
