...
import { setFilter, replaceFilters } from '@ncigdc/utils/filters';
...
const Spinner = () => <Column style={{ alignItems: 'center', padding: '20px' }}><SpinnerParticle /></Column>;
  ...
  lifecycle({
    async componentDidMount(): Promise<*> {
      ...
      this.props.setState(s => ({
        ...
        numCasesAggByProject,
        totalNumCases: Object.keys(numCasesAggByProject).reduce((sum, b) => sum + numCasesAggByProject[b], 0),
      }));
    ...
    <div>
      <Column style={styles.card}>
        ...
        <LocationSubscriber>{(ctx: {| pathname: string, query: TRawQuery |}) => {
          ...
          const componentFilters = replaceFilters(setFilter({
            field: 'cases.project.project_id',
            value: projectId,
          }), currentFilters);

          if (aggregationLoading) return <Spinner />;
          return (
            <FrequentlyMutatedGenesContainer
              ...
              survivalPlotloading={survivalLoading}
            />
          );
        }}</LocationSubscriber>
      ...
      <Column style={{ ...styles.card, marginTop: '2rem', position: 'static' }}>
        ...
        <LocationSubscriber>{(ctx: {| pathname: string, query: TRawQuery |}) => {
          const { filters } = ctx.query || {};
          const currentFilters = parseFilterParam(filters, { op: 'and', content: [] });
          const componentFilters = replaceFilters(setFilter({ field: 'cases.project.project_id', value: projectId }), currentFilters);
          return (
            <OncoGridWrapper
              projectId={projectId}
              currentFilters={componentFilters}
        ...
        <LocationSubscriber>{(ctx: {| pathname: string, query: TRawQuery |}) => {
          ...
          return (
            <FrequentMutationsContainer
              ...
              totalNumCases={totalNumCases}
              projectId={projectId}
              defaultSurvivalRawData={defaultSurvivalData.rawData}
              defaultSurvivalLegend={defaultSurvivalData.legend}
              showSurvivalPlot
              currentFilters={componentFilters}
              survivalPlotloading={survivalLoading}
            />
